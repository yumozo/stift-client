...
const MenuBar = ({ editor, textObj }) => {
  ...
  return (
    <div className='flex flex-row text-editor-buttons pb-4 text-sm justify-between'>
      ...
      <div>
        <button onClick={() => editor.chain().focus().toggleHighlight().run()} className={editor.isActive('bg-yellow-400') ? 'is-active' : ''}>
          duplicate
        </button>
...
const TextEditor: React.FC = () => {
  ...
  return (
    <>
      <div
        className="border-2 border-gray-400/20 px-4 py-2 bg-white"
      >
